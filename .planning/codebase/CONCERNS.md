...
**Address stored as concatenated string then split on read:**
- Issue: Address is written as `${street} ${streetNumber}` (a single string) in `create-event.action.ts:52` and `create-event-form.tsx:177`. On load in `get-event-for-edit.action.ts:47-49`, it is split with `parts.pop()` to recover street number — this fails for multi-word addresses (e.g. "Jana Pawła II 15" splits street number correctly but "ul. Jana Pawła 12A" may not)
- Files: `src/actions/events/create-event.action.ts`, `src/actions/events/update-event.action.ts`, `src/actions/events/get-event-for-edit.action.ts`
- Impact: Data integrity issue; address round-trip parsing is fragile
- Fix approach: Store `street` and `streetNumber` as separate database columns
