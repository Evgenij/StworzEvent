...
function Portal({ className, ...props }: React.ComponentProps<"div">) {
	...
	return createPortal(
		<div
			className={cn(
				"fixed inset-0 isolate z-40 flex flex-col",
				className,
			)}
			{...props}
		/>,
...
function PortalBackdrop({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			className={cn(
				"data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0 -z-1 bg-white duration-500 data-[state=closed]:animate-out data-[state=open]:animate-in",
